...
from dino.backbones import get_model

class PatchvinoModel(DynamicBufferMixin, nn.Module):
    """Patchcore Module.
    ...
    def __init__(
        ...
        num_neighbors: int = 9,input_size=224
    ) -> None:
        super().__init__()
        self.tiler: Tiler | None = None
        dinov2_embedding_sizes = {
            "dinov2_vits14": 384,  # Small model
        ...
        self.embedding_size=dinov2_embedding_sizes[dino_name]
        self.layers = layers
        ...
        self.smaller_edge_size=input_size
    def forward(self, input_tensor: torch.Tensor) -> torch.Tensor | dict[str, torch.Tensor]:
        ...
        if not hasattr(self, 'dino_extractor'):
            model_name = self.dino_name
            self.dino_extractor = get_model(model_name, 'cuda', smaller_edge_size=self.smaller_edge_size)
        output_size = input_tensor.shape[-2:]
        batch_size, _, width, height = input_tensor.shape

        tensor_lst=[]
        for tensor in input_tensor:
            padding_tensor,self.grid_size=self.add_padding(tensor)
            tensor_lst.append(padding_tensor)
        padding_tensors=torch.stack(tensor_lst)

        embedding_dino = self.dino_extractor.extract_features(padding_tensors)
        embedding_reshape = embedding_dino.reshape((batch_size,*self.grid_size, -1)).permute(0,3,1,2)
        ...
        features['dinov2']=embedding_reshape
        features = {layer: self.feature_pooler(feature) for layer, feature in features.items()}

        embedding_patchcore = self.generate_embedding(features)

        if self.tiler:
            embedding_patchcore = self.tiler.untile(embedding_patchcore)

        batch_size, _, width, height = embedding_patchcore.shape
        embedding = self.reshape_embedding(embedding_patchcore)
        if self.training:
            output = embedding
        else:
        ...
        return output
    def add_padding(self, image_tensor: torch.Tensor) -> torch.Tensor:
        ...
        image_tensor = torch.nn.functional.pad(image_tensor, (0, pad_width, 0, pad_height), mode='constant', value=0)

        grid_size = ((height + pad_height) // self.patch_size, (width + pad_width) // self.patch_size)
        return image_tensor, grid_size

    def generate_embedding_dino(self, features: dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    def generate_embedding(self, features: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        Returns:
            Embedding vector
        """
        # embeddings = features[self.layers[1]] # layer 3
        # embeddings = features[self.layers[0]]  # layer 2

        # layer 2 and 3
        embeddings = features[self.layers[0]]  # layer 2
        layer_embedding = features[self.layers[1]]  # layer 3
        layer_embedding = F.interpolate(layer_embedding, size=embeddings.shape[-2:], mode="bilinear")
        embeddings = torch.cat((embeddings, layer_embedding), 1)

        layer_embedding = features['dinov2']
        layer_embedding = F.interpolate(layer_embedding, size=embeddings.shape[-2:], mode="bilinear")
        embeddings = torch.cat((embeddings, layer_embedding), 1)
        return embeddings
