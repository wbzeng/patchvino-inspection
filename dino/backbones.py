...
class ViTWrapper(VisionTransformerWrapper):
    ...
    def extract_features(self, img_tensor):
        with torch.no_grad():
            ...
            return patch_features[:, 1:, :].squeeze() # Exclude the class token
...
class DINOv2Wrapper(VisionTransformerWrapper):
    def load_model(self):
        # model = torch.hub.load(self.model_name)
    ...
    def prepare_image(self, img):
        ...
        pad_height = (self.model.patch_size - height % self.model.patch_size) % self.model.patch_size
        pad_width = (self.model.patch_size - width % self.model.patch_size) % self.model.patch_size

        # Add padding to the image
        image_tensor = torch.nn.functional.pad(image_tensor, (0, pad_width, 0, pad_height), mode='constant', value=0)

        grid_size = ((height + pad_height) // self.model.patch_size, (width + pad_width) // self.model.patch_size)
    ...
    def get_embedding_visualization(self, tokens, grid_size, resized_mask=None, normalize=True):
        ...
        if normalize:
            ...
            scaled_tokens = (normalized_tokens * 255).astype(np.uint8)
